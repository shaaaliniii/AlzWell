...
app.use(flash());
app.use(session({
  secret: 'your-secret-key',  // Provide a secret key here
  resave: false,
...
//location
app.post('/update-location/:userId', async (req, res) => {
  ...
  try {
    ...
    // Push new location into locationHistory
...
app.get('/location', (req, res) => {
  const mapboxToken = 'pk.eyJ1IjoiaWNlLWJlYXIiLCJhIjoiY2xzNXUyZWtyMWo4eDJqbzN6NjZwZmdseSJ9.o139wwZER4wqMr2jFPinvw';
  const userId = req.session.userId || 'USER_ID'; // Replace with actual user ID from session or auth system

  res.render('locationhist.ejs', { mapboxToken, userId });
});
...
app.post('/sendSOS', checkAuthenticated, async (req, res) => {
  try {
    ...
    await client.messages.create({
      ...
      from: '+15558675310', // Your Twilio phone number
      to: emergencyContactPhone
    });
    const message = await client.messages.create({
      body: "Hello there!",
      from: "whatsapp:+15558675310",
      to: "whatsapp:+917879692647",
    });
...
app.get('/nearh', checkAuthenticated, (req, res) => {
  res.render('nearh.ejs');
});
app.get('/predict', checkAuthenticated, (req, res) => {
